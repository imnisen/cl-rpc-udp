...
(defgeneric call (node rpc-name args dest-host dest-port &key timeout)
  (:method ((node rpc-node) rpc-name args dest-host dest-port &key (timeout 5))
    ...
    (let ((msg (make-hash-table :test 'equal))
      ...
      (handler-case
          (usocket:socket-send (node-socket node) buffer (length buffer) :host dest-host :port dest-port)
        (error (e)
          (log:info "node call to dest(~a:~a) error: ~a" dest-host dest-port e)))
  ...
  (labels (;; function to get and extract data, return (success is-request id  remote-host remote-port data) format, data is a list
               ...
               (handler-case
                   (let* ((msg (decode-msg (subseq return-buffer 0 return-length)))
                     ...
                     (cond ((string-equal "request" msg-type)
                            (log:info "node worker get data decode: type:request, id:~a, host:~a, port:~a, data:~a"
                                      (gethash "id" msg) (ip-vector-to-string remote-host) remote-port (list (gethash "rpc-name" msg) (gethash "args" msg)))
                            (list t t (gethash "id" msg) remote-host remote-port (list (gethash "rpc-name" msg) (gethash "args" msg))))

                           ((string-equal "response" msg-type)
                            (log:info "node worker get data decode: type:response, id:~a, host:~a, port:~a, data:~a"
                                      (gethash "id" msg) (ip-vector-to-string remote-host) remote-port (list (gethash "result" msg)))
           ...
           (send-response (socket remote-host remote-port id result)
             (let ((msg (make-hash-table :test 'equal))
               ...
               (setf buffer (encode-msg msg))
               (log:info "node worker send msg-id:~a to remote host: ~a, port: ~a, with result:~a"
                         id
                         (ip-vector-to-string remote-host)
                         remote-port
                         result)
